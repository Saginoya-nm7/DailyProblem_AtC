        ...
        with closing(sqlite3.connect(self.path)) as dbc:
            ...
            if not f:
                ...
                cur.execute("create table problems("
                            "id text, title text, contest_id text, contest_title text, status text, "
                            "point double, point_status text, difficulty double, "
                            "primary key(id))")
    ...
    def appendDBFromJson(self, json):
        ...
        with closing(sqlite3.connect(self.path)) as dbc:
            ...
            for key in json.keys():
                ...
                sql = "insert into " \
                      "problems(id, title, contest_id, contest_title, status, point, point_status, difficulty) " \
                      "values('{}','{}','{}','{}','{}',{},'{}',{})" \
                    .format(key, data["title"], data["contest_id"], data["contest_title"], data["status"],
                            point, data["point_status"], diff)
